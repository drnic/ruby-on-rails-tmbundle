...
# Lets guess it's a method
  ...
  @term = @term + '?' if TextMate.current_line.match(Regexp.escape(@term + '?'))
  # if it's a format of a magic method generated by routes look for it in routes.rb first
  if @term.match(/(new_|edit_)?(.*?)_(path|url)/)
